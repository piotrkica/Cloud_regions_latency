...
class NetworkManager:
    ...
    def cleanup(self, locations: "list[ResourceLocation]"):
        cleanup_resource([self._nic_name(location) for location in locations], [
            partial(lambda x: self._network_client.network_interfaces.begin_delete(x.rg, self._nic_name(x)), location)
            for location in locations
        ])
        cleanup_resource_sequential([self._ip_name(location) for location in locations], [
            partial(lambda x: self._network_client.public_ip_addresses.begin_delete(x.rg, self._ip_name(x)), location)
            for location in locations
        ])
        cleanup_resource([self._subnet_name(location) for location in locations], [
            partial(lambda x: self._network_client.subnets.begin_delete(location.rg, self._vnet_name(x),
                                                                        self._subnet_name(x)), location) for location in
            locations
        ])
        cleanup_resource([self._vnet_name(location) for location in locations], [
            partial(lambda x: self._network_client.virtual_networks.begin_delete(x.rg, self._vnet_name(x)), location)
            for location in locations
        ])
        cleanup_resource([self._nsg_name(location) for location in locations], [
            partial(lambda x: self._network_client.network_security_groups.begin_delete(x.rg, self._nsg_name(x)),
